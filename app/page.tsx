...
export default function Home() {
  ...
  async function enableNotifications() {
    if (!('serviceWorker' in navigator)) {
      alert('Service Worker není podporován.');
      return;
    }

    if (!('Notification' in window)) {
      alert('Notifikace nejsou podporovány.');
      return;
    }
    ...
    if (!response.ok) {
      alert('Notifikace povoleny, ale testovací zprávu se nepodařilo poslat.');
      return;
    }

    alert('Notifikace povoleny a testovací zpráva odeslána.');
  }
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          >
            ...
            <button
              ...
              className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-800 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
            >
              🔔 Povolit notifikace
            </button>
